...
function mysqlgethost(){
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($type, SORT_ASC, $hostips);
...
function addhost(){ 
  ...
  $SQL = "insert into  hostip(hostname,ip,type,status) values('','','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." addhost sql:".$SQL);
...
function updatehost(){ 
  ...
  for ($i=0; $i<count($hostips); $i++) {
      if( !empty($_REQUEST["id".$hostips[$i]['Id']]) ){
           $SQL = "update hostip set ";
           $SQL.= " hostname=\"".$_POST[id.$hostips[$i]['Id'].'hostname']."\",";
           $SQL.= " ip=\"".$_POST[id.$hostips[$i]['Id'].'ip']."\",";
           $SQL.= " type=\"".$_POST[id.$hostips[$i]['Id'].'type']."\","; 
           $SQL.= " status=\"".$_POST[id.$hostips[$i]['Id'].'status']."\""; 
           $SQL.= " where Id=\"".$hostips[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatehost sql:".$SQL);
...
function delhost(){ 
  ...
  for ($i=0; $i<count($hostips); $i++) {
      if( !empty($_REQUEST["id".$hostips[$i]['Id']]) ){
           $SQL = "delete from hostip where Id=\"".$hostips[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." delhost sql:".$SQL);
...
function  show_host_part1(){
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>服务器编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:38%'><div id='rkey'>Hostname:</div></li>\n";
     echo "	<li style='width:20%'><div id='rkey'>Ip:</div></li> \n";		
     echo "	<li style='width:20%'><div id='rkey'>type:</div></li> \n";
     echo "	<li style='width:20%'><div id='rkey'>status:</div></li> \n";
	 echo " </br>\n";

     for ($i=0; $i<count($hostips); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$hostips[$i]['Id']."' />\n";
            echo "	<li style='width:38%'><div id='rvalue'><input style='width:400%' type='text' name='id".$hostips[$i]['Id']."hostname' value='".$hostips[$i]['hostname']."'/></div></li>\n";
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:200%' type='text' name='id".$hostips[$i]['Id']."ip' value='".$hostips[$i]['ip']."'/></div></li> \n";		
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:200%' type='text' name='id".$hostips[$i]['Id']."type' value='".$hostips[$i]['type']."' /></div></li> \n";
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:200%' type='text' name='id".$hostips[$i]['Id']."status' value='".$hostips[$i]['status']."'/></div></li> \n";
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo "<input name='addhost' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='modhost' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='delhost' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
