...
function selectdb(){
	set_include_path(get_include_path() . ':' . "../qinmonitor/");
	...
	$sql="select projname,logdir,logfilename from moninfo.tailproject union select cname as projname,logdir,'catalina.out' as logfilename from docker.dkcreated"; 
	$res=mysql_query($sql,$conn);
    $rows=mysql_affected_rows($conn);//获取行数
    $colums=mysql_num_fields($res);//获取列数
	global $projnames;
...
//如果点击了查看日志，
if( !empty($_REQUEST['tailf']) ){ 
	$lines   = $_REQUEST['lines'];
	$keyword = $_REQUEST['keyword'];
	$selip       =split('-',$_REQUEST["selhostname"]);
	$selprojname =split('-',$_REQUEST["selprojname"]);
	$hostid	= $selip[0];
...
//到达刷新次数限制退出
}elseif( $i > $totaltimes or !empty($_REQUEST['stoptail']) ){ 
	readlog_ctn2($filename,$i,$size2,$ip);
	$cmd="echo '' >/tmp/tail.ctl 2>&1 "; 
	//echo "cmd:".$cmd;
	//echo "i1:".($i > $totaltimes);
	//echo "i2:".!empty($_REQUEST[stoptail]);
//否则继续刷新	
}elseif( $i != $totaltimes){ 
	$freshtime=2;
	$i++;
