...
include_once("dkconfig.php");
	...
	updatedb($sql);
	$sql="LOAD DATA LOCAL INFILE '".$dkctstat_file."' INTO TABLE `dockercontainers` FIELDS TERMINATED BY ','  (`ip`, `cid`, `cname`, `image`, `cmd`, `createtime`, `status`, `ports`)";
	updatedb($sql);
...
// select ip,cid,status from dockercontainers where substring(status,1,2)='Up'
	...
	updatedb($sql);
	$sql="LOAD DATA LOCAL INFILE '".$dkpjstat_file."' INTO TABLE `dkprojstatus` FIELDS TERMINATED BY ','  (`cid`, `pid`)";
	updatedb($sql);	
