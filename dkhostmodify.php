...
function  dkhostrela(){
         if(isset($_POST['adddkhost'])){     
             adddkhost();
         }
         if(isset($_POST['moddkhost'])){  
             mysqlgetdkhost();   
             updatedkhost();
         }
         if(isset($_POST['deldkhost'])){     
             mysqlgetdkhost();
             deldkhost();
         }
         mysqlgetdkhost();
...
function mysqlgetdkhost(){
  ...
  mysql_select_db("docker");
  mysql_query("SET NAMES UTF8");
  $SQL = "SELECT * from dockerhost";
  $res=mysql_query($SQL);
  global $dkhosts;
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($ip, SORT_ASC, $dkhosts);
...
function adddkhost(){ 
  ...
  $SQL = "insert into  dockerhost(ip,ports,hostdir) values('','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." adddkhost sql:".$SQL);
...
function updatedkhost(){ 
  ...
  for ($i=0; $i<count($dkhosts); $i++) {
      if( !empty($_REQUEST["id".$dkhosts[$i]['Id']]) ){
           $SQL = "update dockerhost set ";
           $SQL.= " ip=\"".$_POST[id.$dkhosts[$i]['Id'].'ip']."\",";
           $SQL.= " ports=\"".$_POST[id.$dkhosts[$i]['Id'].'ports']."\",";
           $SQL.= " hostdir=\"".$_POST[id.$dkhosts[$i]['Id'].'hostdir']."\""; 
           $SQL.= " where Id=\"".$dkhosts[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatedkhost sql:".$SQL);
...
function deldkhost(){ 
  ...
  for ($i=0; $i<count($dkhosts); $i++) {
      if( !empty($_REQUEST["id".$dkhosts[$i]['Id']]) ){
           $SQL = "delete from dockerhost where Id=\"".$dkhosts[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." deldkhost sql:".$SQL);
...
function  show_dkhost_part1(){
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>dkhostDocker主机编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:30%'><div id='rkey'>ip:</div></li>\n";
     echo "	<li style='width:38%'><div id='rkey'>ports:</div></li> \n";		
     echo "	<li style='width:30%'><div id='rkey'>hostdir:</div></li> \n";
	 echo " </br>\n";

     for ($i=0; $i<count($dkhosts); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$dkhosts[$i]['Id']."' />\n";
            echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$dkhosts[$i]['Id']."ip' value='".$dkhosts[$i]['ip']."'/></div></li>\n";
         	echo "	<li style='width:38%'><div id='rvalue'><input style='width:300%' type='text' name='id".$dkhosts[$i]['Id']."ports' value='".$dkhosts[$i]['ports']."'/></div></li> \n";		
         	echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$dkhosts[$i]['Id']."hostdir' value='".$dkhosts[$i]['hostdir']."' /></div></li> \n";
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo " </br>\n";	 
     echo "<input name='adddkhost' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='moddkhost' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='deldkhost' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
